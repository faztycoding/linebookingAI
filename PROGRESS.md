...
- Branded menu with 5 tap areas (the large top "จองบริการเพิ่ม" CTA plus the 2x2 grid below), all wired as deterministic `postback` actions (never free text to the AI, consistent with the existing design principle that postback events bypass the AI orchestrator):
  - จองบริการเพิ่ม and เริ่มจอง / ดูเมนู → both use the same deterministic `buildServiceCarouselMessages()` used by `book_again` and `follow`
...
- Redesigned from the initial flat green layout using the approved branded artwork (`ChatGPT Image Aug 26, 2026, 03_34_28 PM.png`). `npm run gen:rich-menu` resizes it to LINE's required 2500x1686 dimensions and exports `public/rich-menu.jpg` at 288 KB; JPEG is used because a photo-style PNG was 4.8 MB and exceeded LINE's 1 MB rich-menu upload limit
- `sharp` remains a dev dependency used only for local image generation and regression metadata checks; the Vercel route reads the pre-generated static JPEG and does not load platform-specific `sharp` binaries at runtime
